...
校正監査画面の「高度な日本語解析」を押したときだけ、MeCab/CaboCha参照解析を実行します。起動・入力・保存・通常校正からは自動実行されず、解析結果から本文を変更または保存することもありません。解析中に本文が変わった場合は古い結果を破棄します。未導入時も通常校正はそのまま利用できます。

macOSの解析環境は、古いビルドツールが空白を含むパスを扱えないため `~/Library/NEXUS/native-japanese` を使用します。その他のOSではアプリのユーザーデータ領域にある `native-japanese` を使用します。開発時だけ `NEXUS_NATIVE_JAPANESE_ROOT` で構築済み環境を指定できます。
